...
class HomeController extends Controller
{
   ...
   public function index(Request $request)
   {
    ...
    try {
        ...
        $logged_in=SmUserLog::where('user_id',$user_id)->where('role_id',1)->where('school_id', Auth::user()->school_id)->get();
        //dd($logged_in->count());
        if($logged_in->count() <= 1 )
        {
            return redirect()->to('/rolepermission/assign-permission/1');
        }
        $totalStudents = SmStudent::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
        ...
        $notices = SmNoticeBoard::select('*')->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();

            // for current month

        $m_add_incomes = SmAddIncome::where('active_status', 1)->where('date', 'like', date('Y-m-') . '%')->where('school_id', Auth::user()->school_id)->sum('amount');

        $m_fees_payments = SmFeesPayment::where('active_status', 1)->where('payment_date', 'like', date('Y-m-') . '%')->where('school_id', Auth::user()->school_id)->sum('amount');

        $m_item_sells = SmItemSell::where('active_status', 1)->where('sell_date', 'like', date('Y-m-') . '%')->where('school_id', Auth::user()->school_id)->sum('total_paid');

        $m_total_income = $m_add_incomes + $m_fees_payments + $m_item_sells;

        $m_add_expenses = SmAddExpense::where('active_status', 1)->where('date', 'like', date('Y-m-') . '%')->where('school_id', Auth::user()->school_id)->sum('amount');
        ...
        $y_add_incomes = SmAddIncome::where('active_status', 1)->where('date', 'like', date('Y-') . '%')->where('school_id', Auth::user()->school_id)->sum('amount');

        $y_fees_payments = SmFeesPayment::where('active_status', 1)->where('payment_date', 'like', date('Y-') . '%')->where('school_id', Auth::user()->school_id)->sum('amount');

        $y_item_sells = SmItemSell::where('active_status', 1)->where('sell_date', 'like', date('Y-') . '%')->where('school_id', Auth::user()->school_id)->sum('total_paid');
        ...
        if (Auth::user()->role_id == 4) {
            ...
            $events = SmEvent::where('active_status', 1)->where('school_id', Auth::user()->school_id)->where('for_whom', 'All')->get();
        }
