...
class SmFrontendController extends Controller
{
    ...
    public function contact()
    {
        ...
        try {
            ...
            $contact_info = SmContactPage::first();
            return view('frontEnd.home.light_contact', compact('contact_info'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function newsPage()
    {
        ...
        try {
            if(Auth::id() != null )
            {
                ...
                $newsPage = SmNewsPage::where('school_id',Auth::user()->school_id)->get();
                if($newsPage->isEmpty()){
                 $news = SmNews::where('id',1)->get(); 
                 $newsPage = SmNewsPage::where('id',1)->get();
             }
    ...
    public function newsHeading()
    {
        ...
        try {
            $SmNewsPage = SmNewsPage::where('school_id',Auth::user()->school_id)->get();
            if($SmNewsPage->isEmpty()){
                $SmNewsPage=SmNewsPage::find(1); 
            }
    ...
    public function register(Request $request,$school_Id1)
    {

        try {
            $last_id=Crypt::decrypt($school_Id1);

            $schools = SmSchool::where('id', $last_id)->get();
            $roles=Role::get();
            
            return view('auth.registerCodeCanyon', compact('schools','roles','last_id'));
        } catch (\Exception $e) {
            dd($e);
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
    ...
    public function customer_register(Request $request)
    {
        ...
        try {
            ...
            $s = new User();
            $s->school_id=$request->school_id;
            $s->role_id = $request->role_id;
            ...
            if (!empty($result)) {
                ...
                Mail::send(['html' => 'backEnd.studentInformation.user_credential'], compact('data'), function ($message) {
                    ...
                    $message->to($latestSchool->email, $Schoolname)->cc("dmmalik@ymail.com")->subject('Login Details');

                });
                Toastr::success('Operation successful', 'Success');
                return redirect()->route('login', [$sid])->with('Success',"Please check your mail for Department register");
            } else {
                Toastr::error('Operation Failed', 'Failed');
                return redirect()->back();
            }
