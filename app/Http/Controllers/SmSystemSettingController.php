...
class SmSystemSettingController extends Controller
{
    ...
    public function generalSettingsView(Request $request)
    {
        ...
        try {
            $editData = SmGeneralSettings::where('id',Auth::user()->school_id)->first();
            $session = SmGeneralSettings::join('sm_academic_years', 'sm_academic_years.id', '=', 'sm_general_settings.session_id')->where('sm_general_settings.school_id',Auth::user()->school_id)->first();
            // return $editData;
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
