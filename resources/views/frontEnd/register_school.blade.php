			...
			<div class="row login-height justify-content-center align-items-center">
				<div class="col-lg-5 col-md-8">
					<div class="form-wrap text-center in_login_content">
						...
						<form method="post" class="" action="{{url('register_school')}}" enctype="multipart/form-data">
                        @csrf
						<?php if(session()->has('message-danger') != ""): ?>
							...
							<div class="form-group input-group">
								...
								<input class="form-control{{ $errors->has('school_name') ? ' is-invalid' : '' }}" type="text" name='school_name' placeholder="Enter your School Name" required="required" />
							...
							<div class="form-group input-group">
								...
								<input class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" type="email" name='email' placeholder="Enter Email address" required="required" />
							...
							<div class="form-group input-group">
								...
								<input class="form-control{{ $errors->has('phone') ? ' is-invalid' : '' }}" type="text" name='phone' placeholder="Enter Phone Number" required="required" />
							...
							<div class="form-group input-group">
								...
								<input class="form-control{{ $errors->has('address') ? ' is-invalid' : '' }}" type="textarea" name='address' placeholder="Enter address" required="required" />
								
							</div>
							
							 <div class="form-group input-group">
								<label for="logo_img">Upload School Logo</label>
								<input type="file" name="logo_img" class="form-control{{ $errors->has('logo_img') ? ' is-invalid' : '' }}"   id="logo_img">

								
							</div>
								<div class="input-effect mb-40 in_single_input">
                                        <select class="mb-26 niceSelect infix_theme_style w-100 bb form-control{{ $errors->has('school_type') ? ' is-invalid' : '' }}" name="school_type" >
                                            
                                            @foreach($schools_type as $school)
                                                <option value="{{$school->id}}"> {{$school->school_type}} </option>
                                            @endforeach
                                        </select>
                                    </div>
							</div>
