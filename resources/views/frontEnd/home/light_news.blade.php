...
@section('main_content')
    ...
    <section class="container box-1420">
        <div class="banner-area" style="background: linear-gradient(0deg, rgba(124, 50, 255, 0.6), rgba(199, 56, 216, 0.6)), url({{$newsPage[0]->image != ""? $newsPage[0]->image : '../img/client/common-banner1.jpg'}}) no-repeat center;">
            <div class="banner-inner">
                <div class="banner-content">
                    <h2>{{$newsPage[0]->title}}</h2>
                    <p>{{$newsPage[0]->description}}</p>
                    <a class="primary-btn fix-gr-bg semi-large" href="{{$newsPage[0]->button_url}}">{{$newsPage[0]->button_text}}</a>
                </div>
            </div>
        </div>
    </section>
